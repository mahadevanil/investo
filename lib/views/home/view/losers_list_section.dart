...
class LosersListSection extends StatelessWidget {
  const LosersListSection({
    super.key,
  });
    ...
    return Consumer<HomeControl>(builder: (context, homeControl, _) {
      return SizedBox(
        ...
        child: homeControl.apiLoading == true
            ...
            : homeControl.marketListResp.topLosers != null &&
                    homeControl.marketListResp.topLosers!.isNotEmpty
                ? ListView.builder(
                    physics: const NeverScrollableScrollPhysics(),
                    shrinkWrap: true,
                    scrollDirection: Axis.vertical,
                    itemCount:
                        homeControl.marketListResp.topLosers?.length ?? 0,
                    itemBuilder: (context, index) {
                      var data = homeControl.marketListResp.topLosers?[index];
                      return InkWell(
                        onTap: () {
